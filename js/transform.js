...
var MVPmat = function ( dispParams ) {
	...
	function computeViewTransform( state, halfIpdShift ) {
		...
		viewerPosition.x = -1.0 * (halfIpdShift);
		viewerPosition.y = 0.0;
		viewerPosition.z = dispParams.distanceScreenViewer;

		var viewerTarget = state.viewerTarget;
		viewerTarget.x = -1.0 * (halfIpdShift);
		viewerTarget.y = 0.0;
		viewerTarget.z = 0.0;
	...
	function update( state, renderingMode ) {
		...
		if ( renderingMode === STANDARD_MODE
			...
			var top = state.clipNear * ((dispParams.canvasHeight * dispParams.pixelPitch) / (2.0 * dispParams.distanceScreenViewer));
			var bottom = -1.0 * top;

			var right_eyeL = (state.clipNear) * ( ((dispParams.canvasWidth * dispParams.pixelPitch) + dispParams.ipd) / (2.0 * dispParams.distanceScreenViewer)	);
			var left_eyeL = -1.0 * (state.clipNear) * (	((dispParams.canvasWidth * dispParams.pixelPitch) - dispParams.ipd) / (2.0 * dispParams.distanceScreenViewer)	);

			var right_eyeR = state.clipNear * (	((dispParams.canvasWidth * dispParams.pixelPitch) - dispParams.ipd) / (2.0 * dispParams.distanceScreenViewer) );
			var left_eyeR = -1.0 * state.clipNear * ( ((dispParams.canvasWidth * dispParams.pixelPitch) + dispParams.ipd) / (2.0 * dispParams.distanceScreenViewer) );



			this.anaglyphProjectionMat.L = computePerspectiveTransform(
				left_eyeL, right_eyeL, top, bottom, state.clipNear, state.clipFar );

			this.anaglyphProjectionMat.R = computePerspectiveTransform(
				left_eyeR, right_eyeR, top, bottom, state.clipNear, state.clipFar );
