...
var FoveatedRenderer = function ( webglRenderer, dispParams ) {
	...
	function computePixelVA( pixelPitch, distanceScreenViewer ) {
		...
		console.log("pixel pitch param: ", pixelPitch)
		console.log("distance param: ", distanceScreenViewer)
		var rads = 2 * Math.atan(pixelPitch / (2 * distanceScreenViewer));
		console.log("Radians: ", rads, "\n")
		var degs = (rads * 180) / Math.PI;
		console.log("Degrees: ", degs, "\n")
		// console.log("texture map?: ", this.renderTarget.texture)

		// middleBlurKernel = [ 0.0625, 0.2500, 0.3750, 0.2500, 0.0625 ]
		// middleBlurKernel2d = new Array(25);
		// dmidc = 0;
// for(i = 0; i < 5; i++)
// {
// 	for(j = 0; j < 5; j++)
...
// 	}
// }
// console.log('outer prod? ', middleBlurKernel2d)
	...
	function computeEcc( mar ) {
		console.log("incoming mar: ", mar)	
		/* TODO (2.2.2) Eccentricity for some MAR */
		var lil_m = 0.0275
		var omega_oh = 1/48
		var eccentricRet = 0

		var lhs = mar - omega_oh
		lhs = lhs / lil_m
		eccentricRet = lhs
		console.log("eccentricity angle, ", eccentricRet)
		return eccentricRet;
